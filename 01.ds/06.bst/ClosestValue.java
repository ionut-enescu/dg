/**
 * Given a binary search tree (BST) and a target number, find a node value in
 * the BST that is closest to the given target. If there are multiple answers,
 * print the smallest.
 * 
 * A BST is a tree where for every node, the values in the left subtree are
 * smaller than the node, and the values in the right subtree are greater.
 * 
 * Examples
 * Example 1:
 ...
 * 10 30
 * Expected Output: 20
 * Justification: 20 and 30 are the closest numbers to 25. However, 20 is
 * smaller than 30. So, 20 is selected as an output.
 * Example 3:
...
// this can be O(n) for a skewed tree. In a balanced tree, it would be O(log n).
// Space Complexity: O(h) for the recursion stack, where h is the height of the
// tree. In a balanced tree, this would be O(log n), and in a skewed tree, it
// would be O(n).
